...
// ── Estado del pedido en vivo (polling cada 15s) ──────────────────────────────
const { useFindOrder } = useOrders()
const { data: liveOrder } = useFindOrder(() => props.order.id)
const currentStatus = computed(() => liveOrder.value?.status ?? props.order.status)
...
const statusSteps = computed(() => [
  ...
  { key: 'delivered', label: props.order.type === 'delivery' ? 'En camino' : 'Servido', icon: 'i-lucide-check-check' }
])
const currentStepIndex = computed(() => {
  const i = statusSteps.value.findIndex(s => s.key === currentStatus.value)
  return i < 0 ? 0 : i
})
